...
BEGIN
    ...
    IF TG_OP = 'INSERT' THEN
		...
		-- DECREMENTAR ESTOQUE NA TABELA LIVRO
        UPDATE LIVRO SET QUANT_ESTOQUE = QUANT_ESTOQUE - NEW.QUANT_ITEM 
		WHERE LIVRO.COD_LIVRO = NEW.COD_LIVRO;
		
		-- ATUALIZAR QUANTIDADE DE ITENS DO PEDIDO E VALOR TOTAL DO PEDIDO NA TABELA PEDIDO
		UPDATE PEDIDO SET QUANT_ITENS_PEDIDO = QUANT_ITENS_PEDIDO + NEW.QUANT_ITEM,
		VALOR_TOTAL_PEDIDO = VALOR_TOTAL_PEDIDO + NEW.VALOR_TOTAL_ITEM 
		WHERE PEDIDO.COD_PEDIDO = NEW.COD_PEDIDO;
		    
	-- REMOVER ITENS DO PEDIDO
    ELSIF TG_OP = 'DELETE' THEN
        
		-- INCREMENTAR ITENS EM ESTOQUE NA TABELA LIVRO
		UPDATE LIVRO SET QUANT_ESTOQUE = QUANT_ESTOQUE + OLD.QUANT_ITEM
		WHERE LIVRO.COD_LIVRO = OLD.COD_LIVRO;
		
		-- DECREMENTAR VALOR TOTAL DO PEDIDO E QUANTIDADE DE ITENS DO PEDIDO NA TABELA PEDIDO
		UPDATE PEDIDO SET VALOR_TOTAL_PEDIDO = VALOR_TOTAL_PEDIDO - OLD.VALOR_TOTAL_ITEM,
		QUANT_ITENS_PEDIDO = QUANT_ITENS_PEDIDO - OLD.QUANT_ITEM;
...
CREATE TABLE CONTROLE_ALTERACAO (
	COD_ALTERACAO SERIAL PRIMARY KEY,
    OPERACAO VARCHAR(1),
    DATA_HORA TIMESTAMP,
...
CREATE FUNCTION REGISTRAR_UPDATE_LIVRO()
RETURNS TRIGGER AS $$
BEGIN
	INSERT INTO CONTROLE_ALTERACAO (OPERACAO, DATA_HORA, USUARIO_QUE_ALTEROU, COD_ANTIGO, COD_NOVO,
	ESTOQUE_ANTIGO, ESTOQUE_NOVO) VALUES 
	('U', CURRENT_TIMESTAMP, SESSION_USER, OLD.COD_TITULO, NEW.COD_TITULO, OLD.QUANT_ESTOQUE, 
	 NEW.QUANT_ESTOQUE);
	RETURN NEW;
END;
$$ LANGUAGE plpgsql;
	
CREATE TRIGGER TRIGGER_UPDATE_LIVRO AFTER UPDATE ON LIVRO 
FOR EACH ROW EXECUTE PROCEDURE REGISTRAR_UPDATE_LIVRO();

CREATE FUNCTION REGISTRAR_DELETE_LIVRO()
RETURNS TRIGGER AS $$
BEGIN
	INSERT INTO CONTROLE_ALTERACAO (OPERACAO, DATA_HORA, USUARIO_QUE_ALTEROU, COD_ANTIGO, ESTOQUE_ANTIGO)
	VALUES ('D', CURRENT_TIMESTAMP, SESSION_USER, OLD.COD_TITULO, OLD.QUANT_ESTOQUE);
	RETURN OLD;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER TRIGGER_DELETE_LIVRO AFTER DELETE ON LIVRO 
FOR EACH ROW EXECUTE PROCEDURE REGISTRAR_DELETE_LIVRO();

UPDATE LIVRO SET COD_TITULO = 3, QUANT_ESTOQUE = 60 WHERE COD_LIVRO = 1;

DELETE FROM LIVRO WHERE COD_LIVRO = 7;

INSERT INTO LIVRO (COD_TITULO, VALOR_UNITARIO, QUANT_ESTOQUE) VALUES (1, 100, 50);

SELECT * FROM LIVRO;

SELECT * FROM controle_alteracao;
